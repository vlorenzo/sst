...
def audio_processor(app):
    ...
    while True:
        try:
            ...
            save_audio_segment(audio_chunk, SAMPLE_RATE, input_lang, f"raw-audio_chunk_{timestamp}")
            #save_audio_segment(audio_buffer, SAMPLE_RATE, input_lang, f"audio_buffer_{timestamp}")
            
            # Ensure buffer is correctly updated after processing each chunk
            ...
            if mean_amplitude > silence_threshold:
                ...
                # Preserve the last part of the buffer for the next chunk if overlapping is enabled
                if app.config['USE_OVERLAPPING_CHUNKS']:
                    logger.info("Overlapping is enabled: Preserve the last part of the buffer for the next chunk. ")
                    overlap_duration = app.config['OVERLAP_DURATION']
                    audio_buffer = audio_buffer[-overlap_duration * SAMPLE_RATE:]
                else:
                    logger.info("Overlapping NOT enabled: reset audio_buffer.")
                    audio_buffer = np.array([], dtype=AUDIO_DTYPE)  # Clear the buffer for non-overlapping processing
